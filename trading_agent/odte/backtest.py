...
- Bracket: +take_profit_pct / −stop_loss_pct on that premium (default +20% / −12.5%).
...
from trading_agent.odte.playbook import OdtePlaybookConfig, rsi_series, whole_dollar_levels
...
def fetch_qqq_1m(symbol: str = "QQQ", period: str = "7d"):
...
def run_odte_backtest(
    ...
    *,
    ...
    cfg.symbol = symbol
    if df is None:
        df = fetch_qqq_1m(symbol, period=period)
        ...
        for i, ts in enumerate(times):
            ...
            for name, lvl, kind in candidates:
                ...
                if kind == "resistance" and rsi >= cfg.put_rsi:
                    side = "PUT"
                elif kind == "support" and rsi <= cfg.call_rsi:
                    side = "CALL"
                if side is None:
                    continue
                open_trade = {
    ...
    return OdteBacktestResult(
        ...
        trades=trades,
        assumptions=[
            f"1m yfinance history period={period}",
