...
def run_oms_consume(
    *,
    ...
    is_ibkr = False
    try:
        is_ibkr = str(mx.broker_name() or "").lower() == "ibkr"
    except Exception:
        is_ibkr = False
    ...
    if live or cfg.require_account_cash:
        if is_ibkr:
            try:
                ...
                account_resp = fetch_ibkr_account_balances()
            except Exception as exc:  # noqa: BLE001
                account_resp = {"error": "ibkr_balances_exception", "message": str(exc)}
        else:
    ...
    # (skipped for IBKR paper — Gateway health is separate)
    schwab_block = ""
    if live and not is_ibkr:
        try:
